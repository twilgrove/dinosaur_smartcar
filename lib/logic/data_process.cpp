...
LoopKind Judge::TrackKind_Judge(Img_Store *Img_Store_p, Data_Path *Data_Path_p, Function_EN *Function_EN_p)
{
    ...
    State = Img_Store_p->ImgNum;//时间戳起始帧

    if (Function_EN_p->Control_EN == false)
    {
        Judge::InflectionPointSearch(Img_Store_p, Data_Path_p);//拐点
        Judge::BendPointSearch(Img_Store_p, Data_Path_p);//弯点
        if ((Data_Path_p->Circle_Track_Step == OUT_PREPARE||Data_Path_p->Circle_Track_Step == OUT)&&(Data_Path_p->InflectionPointNum[0] >=1 || Data_Path_p->InflectionPointNum[1] >= 1) )
        {
        ...
        // 十字判断
        // 若左右边线都有拐点则为十字
        else if ((Data_Path_p->InflectionPointNum[0] >= 1) && (Data_Path_p->InflectionPointNum[1] >= 1) && JSON_FunctionConfigData.AcrossIdentify_EN == true && Function_EN_p->Gyroscope_EN == false && (Data_Path_p->Circle_Track_Step != OUT_PREPARE || Data_Path_p->Circle_Track_Step != OUT))
        {
        ...
        else if ((Data_Path_p->InflectionPointNum[0] == 0) && (Data_Path_p->InflectionPointNum[1] >= 1) && (Data_Path_p->BendPointNum[0] <= 6) && (Data_Path_p->BendPointNum[1] >= 0) && State - State_Across >= 5 && Function_EN_p->Gyroscope_EN == false && JSON_FunctionConfigData.CircleIdentify_EN == true&&Data_Path_p->get_dandiao!=0)
        {
            // 准备入环阶段
            // 在出环后经过固定帧数才能再次准备进环
            if (((Data_Path_p->Circle_Track_Step) == INIT || (Data_Path_p->Circle_Track_Step) == IN_PREPARE || (Data_Path_p->Circle_Track_Step) == IN) && Data_Path_p->Vector_Add_Unit_Dir[1] == 1&&Data_Path_p->TrackCoordinate[NumSearch-20][0]>120&&Data_Path_p->TrackCoordinate[NumSearch-20][0]<200)
            {
                Loop_Kind = R_CIRCLE_TRACK_LOOP;
                Data_Path_p->Track_Kind = R_CIRCLE_TRACK_OUTSIDE;
                ...
                Data_Path_p->Circle_in_start=right_white_hang;
            }
            // 入环阶段
        ...
        else if ((Data_Path_p->InflectionPointNum[0] >= 1) && (Data_Path_p->InflectionPointNum[1] == 0) && (Data_Path_p->BendPointNum[0] >= 1) && (Data_Path_p->BendPointNum[1] <= 6) && State - State_Across >= 5 && Function_EN_p->Gyroscope_EN == false && JSON_FunctionConfigData.CircleIdentify_EN == true)
        {
            // 准备入环阶段
            // 在出环后经过固定帧数才能再次准备进环
        ...
        else
        {
            ...
            // 判定圆环步骤
            // 进入圆环后固定帧数进入准备出环步骤
            if (State - State_Circle_IN >= 50 && Data_Path_p->Circle_Track_Step == IN)//State - State_Circle_IN >= 10
            {
            ...
            // 出环后进入出环转直线
            if ((Data_Path_p->Circle_Track_Step) == OUT && State-State_Circle_OUT>150&&Data_Path_p->TrackCoordinate[NumSearch-10][0]>120&&Data_Path_p->TrackCoordinate[NumSearch-10][0]<200)
            {
                Data_Path_p->Circle_Track_Step = OUT_2_STRIGHT;
                State_Circle_OUT = Img_Store_p->ImgNum;
            }
            // 经过固定帧数后出环转直线进入占位从而可以进行准备下一次的圆环
            if ((Data_Path_p->Circle_Track_Step) == OUT_2_STRIGHT && State - State_Circle_OUT >= 200)
            {
                Data_Path_p->Circle_Track_Step = INIT;
            }
...
{
    JSON_TrackConfigData JSON_TrackConfigData = Data_Path_p->JSON_TrackConfigData_v[0];
    // if(Data_Path_p->SideCoordinate[(JSON_TrackConfigData.Forward) - (JSON_TrackConfigData.Path_Search_Start)][0]<=4&&Data_Path_p->SideCoordinate[(JSON_TrackConfigData.Forward) - (JSON_TrackConfigData.Path_Search_Start)][2]>=150)
    // {
    //     Data_Path_p->ServoAngle=Data_Path_p->SideCoordinate[(JSON_TrackConfigData.Forward) - (JSON_TrackConfigData.Path_Search_Start)][2]-130;
    // }
    ...
    // if(Data_Path_p->ServoAngle>=150)
    //     (Data_Path_p->ServoAngle) =150; 
}
...
void Judge::InflectionPointSearch(Img_Store *Img_Store_p, Data_Path *Data_Path_p)
{
    ...
    for (i = (JSON_TrackConfigData.InflectionPointVectorDistance); i <= (Data_Path_p->NumSearch[0]) - (JSON_TrackConfigData.InflectionPointVectorDistance);)
    {
        ...
        if (abs(AngleVector[0]) > (JSON_TrackConfigData.InflectionPointIdentifyAngle[0]) && abs(AngleVector[0]) < (JSON_TrackConfigData.InflectionPointIdentifyAngle[1]) && (Data_Path_p->SideCoordinate_Eight[i][0]) > 15 && (Vector[0][1] * Vector[1][1]) >= -40)
        {
    ...
    for (j = (JSON_TrackConfigData.InflectionPointVectorDistance); j <= (Data_Path_p->NumSearch[1]) - (JSON_TrackConfigData.InflectionPointVectorDistance);)
    {
        ...
        if (abs(AngleVector[1]) > (JSON_TrackConfigData.InflectionPointIdentifyAngle[0]) && abs(AngleVector[1]) < (JSON_TrackConfigData.InflectionPointIdentifyAngle[1]) && 159 - (Data_Path_p->SideCoordinate_Eight[j][2]) > 15 && (Vector[0][3] * Vector[1][3]) >= -40)
        {
...
void Judge::BendPointSearch(Img_Store *Img_Store_p, Data_Path *Data_Path_p)
{
    ...
    for (i = (JSON_TrackConfigData.BendPointVectorDistance); i <= (Data_Path_p->NumSearch[0]) - (JSON_TrackConfigData.BendPointVectorDistance);)
    {
        ...
        if (abs(AngleVector[0]) > (JSON_TrackConfigData.BendPointIdentifyAngle[0]) && abs(AngleVector[0]) < (JSON_TrackConfigData.BendPointIdentifyAngle[1]) && (Data_Path_p->SideCoordinate_Eight[i][0]) > 15)
        {
            //  cout << abs(AngleVector[0]) << endl;
    ...
    for (j = (JSON_TrackConfigData.BendPointVectorDistance); j <= (Data_Path_p->NumSearch[1]) - (JSON_TrackConfigData.BendPointVectorDistance);)
    {
        ...
        if (abs(AngleVector[1]) > (JSON_TrackConfigData.BendPointIdentifyAngle[0]) && abs(AngleVector[1]) < (JSON_TrackConfigData.BendPointIdentifyAngle[1]) && 159 - (Data_Path_p->SideCoordinate_Eight[j][2]) > 15)
        {
            // cout << abs(AngleVector[1]) << endl;
...
//     {
//         circle((Img_Store_p->Img_Color_Unpivot), Point(Circle[i][0.0], Circle[i][1.0]), Circle[i][2.0], Scalar(255, 0, 255), 2);
//     }
// }
