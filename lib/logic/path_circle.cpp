...
void CircleTrack_Step_IN_Prepare(Img_Store *Img_Store_p, Data_Path *Data_Path_p)
{
    ...
    case L_CIRCLE_TRACK_OUTSIDE:
    {
        // 准备左入环补线
        // 赛道彩色图像
        //line((Img_Store_p->Img_Track), Point((Data_Path_p->SideCoordinate_Eight[1][0]), (Data_Path_p->SideCoordinate_Eight[1][1])), Point(int(160 - (JSON_TrackConfigData.TrackWidth) / 2), (Data_Path_p->SideCoordinate_Eight[(Data_Path_p->NumSearch[0]) - 1][1])), Scalar(128, 0, 128), 4);
        // 赛道二值化图像
        //line((Img_Store_p->Img_OTSU), Point((Data_Path_p->SideCoordinate_Eight[1][0]), (Data_Path_p->SideCoordinate_Eight[1][1])), Point(int(160 - (JSON_TrackConfigData.TrackWidth) / 2), (Data_Path_p->SideCoordinate_Eight[(Data_Path_p->NumSearch[0]) - 1][1])), Scalar(255), 4);
    ...
    case R_CIRCLE_TRACK_OUTSIDE:
    {
        // 准备右入环补线
        // 赛道彩色图像
        line((Img_Store_p->Img_Track), Point((Data_Path_p->SideCoordinate_Eight[Data_Path_p->Circle_in_start][2]), (Data_Path_p->SideCoordinate_Eight[Data_Path_p->Circle_in_start][3])), Point(Data_Path_p->SideCoordinate[Data_Path_p->get_dandiao][2],Data_Path_p->SideCoordinate[Data_Path_p->get_dandiao][3]), Scalar(128, 0, 128), 4);
        // 赛道二值化图像
        line((Img_Store_p->Img_OTSU), Point((Data_Path_p->SideCoordinate_Eight[Data_Path_p->Circle_in_start][2]), (Data_Path_p->SideCoordinate_Eight[Data_Path_p->Circle_in_start][3])), Point(Data_Path_p->SideCoordinate[Data_Path_p->get_dandiao][2],Data_Path_p->SideCoordinate[Data_Path_p->get_dandiao][3]), Scalar(255), 4);
