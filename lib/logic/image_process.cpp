...
void ImgProcess::ImgPrepare(Img_Store *Img_Store_p, Data_Path *Data_Path_p, Function_EN *Function_EN_p)
{
	...
	for (int i = 1; i <= JSON_TrackConfigData.DilateErode_Factor[0]; i++)
	{
		erode((Img_Store_p->Img_OTSU), (Img_Store_p->Img_OTSU), (Img_Store_p->Erode_Kernel));
	}
	// 加白框防止八邻域寻线出错
	line((Img_Store_p->Img_OTSU), Point(0, 0), Point(159, 0), Scalar(255), 3);
	line((Img_Store_p->Img_OTSU), Point(159, 0), Point(159, 119), Scalar(255), 3);
	line((Img_Store_p->Img_OTSU), Point(159, 119), Point(0, 119), Scalar(255), 3);
	line((Img_Store_p->Img_OTSU), Point(0, 119), Point(0, 0), Scalar(255), 3);
}
...
void ImgProcess::ImgSobel(Mat &Img)
{
	...
	// 对X方向微分
	Sobel(Img, ImgX, CV_16S, 1, 0, 3); // x方向差分阶数 y方向差分阶数 核大小
	convertScaleAbs(ImgX, ImgX);	   // 可将任意类型的数据转化为CV_8UC1
	// 对Y方向微分
	Sobel(Img, ImgY, CV_16S, 0, 1, 3);		   // x方向差分阶数 y方向差分阶数 核大小
...
void ImgProcess::ImgUnpivot(Mat Img, Mat &Img_Unpivot)
{
	...
	warpPerspective(Img, Img_Unpivot, UnpivotMat, Size(160, 120), INTER_LINEAR);
}
...
void ImgProcess::ImgReferenceLine(Img_Store *Img_Store_p, Data_Path *Data_Path_p)
{
	...
	line((Img_Store_p->Img_Track), Point(0, 119 - (JSON_TrackConfigData.Path_Search_Start)), Point(159, 119 - (JSON_TrackConfigData.Path_Search_Start)), Scalar(0, 0, 255), 1);	  // 寻路径起始线
	line((Img_Store_p->Img_Track), Point(0, 119 - (JSON_TrackConfigData.Path_Search_End)), Point(159, 119 - (JSON_TrackConfigData.Path_Search_End)), Scalar(0, 0, 255), 1);		  // 寻路径结束线
	line((Img_Store_p->Img_Track), Point(0, 119 - (JSON_TrackConfigData.Side_Search_Start)), Point(159, 119 - (JSON_TrackConfigData.Side_Search_Start)), Scalar(255, 0, 255), 1); // 寻边线起始线
	line((Img_Store_p->Img_Track), Point(0, 119 - (JSON_TrackConfigData.Side_Search_End)), Point(159, 119 - (JSON_TrackConfigData.Side_Search_End)), Scalar(255, 0, 255), 1);	  // 寻边线结束线
	line((Img_Store_p->Img_Track), Point(160, 119 - (JSON_TrackConfigData.Path_Search_Start)), Point(160, 119 - (JSON_TrackConfigData.Path_Search_End)), Scalar(0, 0, 255), 1);	  // 中心竖线
}
