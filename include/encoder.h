...
#define PWM_BASE_ADDR 0x1611B000 // 编码器PWM控制器基地址（4个，每个16B）
#define PWM_OFFSET 0x10          // 每个PWM控制器偏移量

#define LOW_BUFFER_OFFSET 0x4  // 低脉冲缓冲寄存器偏移量
#define FULL_BUFFER_OFFSET 0x8 // 脉冲周期缓冲寄存器偏移量
#define CONTROL_REG_OFFSET 0xC // 控制寄存器偏移量
...
{
public:
    ENCODER(int pwmNum, int gpioNum); // 构造函数
    ~ENCODER(void);                   // 析构函数

    double pulse_counter_update(void); // 更新脉冲计数器
