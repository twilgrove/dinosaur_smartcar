...
/* 
    注意事项：如果使用硬件SPI的方式，拓展板对应使用的是SPI1端口，需要在设备树中配置好SPI1才可以使用。
             并且需要提前查看SPI1对应设备文件，与端口初始化处文件名称是否一致
 */
#define USE_QSPI 1 // 0为使用软件SPI的方式，1为使用硬件SPI的方式
...
void TFTSPI_P16x16Str(unsigned char x, unsigned char y, unsigned char *s_dat, unsigned short word_color, unsigned short back_color)
{
    ...
    while (s_dat[ii] != '\0')
    {
        ...
        {
            if (hanzi_Idx[wm] == (unsigned char)s_dat[ii])
            {
...
void Test_TFT18(void)
{
    ...
    cout << "TFTSPI_P16x16Str" << endl;
    TFTSPI_P16x16Str(0, 0, (unsigned char *)"北京龙邱智能科技", u16RED, u16BLUE); // 字符串显示
    cout << "TFTSPI_P8X16Str" << endl;
    TFTSPI_P8X16Str(0, 1, (char *)"Long Qiu i.s.t.", (unsigned short)u16WHITE, (unsigned short)u16BLACK); // 字符串显示
    usleep(1000000);                                                                                      // 延时等待
    char txt[32];
